  ...
  return (
    <div className="max-w-7xl mx-auto p-6">
      <div className="mb-8">
        ...
        <p className="text-gray-600">Theo dõi và cập nhật trạng thái các lô thuốc đang vận chuyển</p>
      </div>
        ...
        <div className="lg:col-span-2">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Danh sách các NFT thuốc đang trong quyền sở hữu</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="text-center py-8 text-gray-500">
                <Package className="w-12 h-12 mx-auto mb-2 opacity-50" />
                <p>Chưa có lô thuốc nào được giao cho bạn</p>
                <p className="text-sm">Các lô thuốc sẽ hiển thị ở đây khi được chuyển giao</p>
              </div>
            </CardContent>
          </Card>
        </div>
        ...
        <div>
          <Card>
            <CardHeader>
              ...
              <CardDescription>Gắn dữ liệu cảm biến vào NFT đã chọn</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              {selectedNFT ? (
                ...
                <div className="p-3 bg-gray-50 rounded-lg">
                  <p className="text-sm text-gray-600">Chọn một lô thuốc để upload dữ liệu</p>
                </div>
              )}
              ...
              <div>
                ...
                />
                {sensorFile && <p className="text-sm text-green-600 mt-1">✓ Đã chọn: {sensorFile.name}</p>}
              </div>
            ...
            <CardContent>
              <div className="space-y-3">
                ...
                <div className="flex justify-between">
                  ...
                  <Badge variant="outline">{mockNFTs.filter((nft) => nft.status === "in_transit").length}</Badge>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">Đã nhận:</span>
                  <Badge variant="outline">{mockNFTs.filter((nft) => nft.status === "received").length}</Badge>
                </div>
              </div>
            </CardContent>
          </Card>
...
export default function DistributorPage() {
  return (
    <RoleGuard requiredRoles={["ADMIN", "DISTRIBUTOR"]}>
      <DistributorContent />
    </RoleGuard>
  )
}
