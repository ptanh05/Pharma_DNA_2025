...
Written in Python using Boa framework (neo3-boa v1.1.1)
...
# storage helpers (use key first, then ctx)
def read_bytes(ctx, key) -> bytes:
    b: bytes = get(key, ctx)
    return b

def write_bytes(ctx, key, value: bytes):
    put(key, value, ctx)

def read_int(ctx, key, length = CNT_BYTES_LEN) -> int:
    b: bytes = get(key, ctx)
    return bytes_to_int(b)

def write_int(ctx, key, value: int, length = CNT_BYTES_LEN):
    put(key, int_to_bytes(value, length), ctx)

def read_bool(ctx, key) -> bool:
    b: bytes = get(key, ctx)
    if b is None or len(b) == 0:
        return False
    return b[0] == 1

def write_bool(ctx, key, val: bool):
    put(key, b'\x01' if val else b'\x00', ctx)

def tokenid_to_key(token_id: int) -> bytes:
...
def get_owner() -> UInt160:
    ctx = get_context()
    b: bytes = read_bytes(ctx, OWNER_KEY)
